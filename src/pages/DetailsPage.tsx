...
export default function DetailsPage() {
  ...
  if (book && book.subjects) {
    ...
    subjects = book.workData.subjects.map((subject, index) => (
      <div key={index} className={styles.featureItem}>
        <IconHeart size="1.05rem" className={styles.icon} stroke={1.5} />
        <p>{subject}</p>
      </div>
  ...
  return (
          ...
          {book.authorData ? (
            <>
              ...
              {book.authorData.birth_date && (
                <p className={styles.label}>Date de naissance : {book.authorData.birth_date}</p>
              )}
              {book.authorData.death_date && (
                <p className={styles.label}>Date de décès : {book.authorData.death_date}</p>
              )}
                {book.authorData.bio && (
                  <p className={styles.label}>
                    Biographie : {typeof book.authorData.bio === 'string' ? book.authorData.bio : book.authorData.bio[0]?.value}
                  </p>
                )}
    ...
    <div className={styles.containerBottom}>
     <div className={styles.section}>
        ...
        {book.publish_places && (
          <p className={styles.label}>Lieu de publication : {book.publish_places}</p>
        )}
        <p></p>
        {book.publish_date && (
          <p className={styles.label}>Date de publication : {book.publish_date}</p>
        )} 
        <p></p>
        {book.workData.description && (
            <p className={styles.label}>Description : {typeof book.workData.description === 'string' ? book.workData.description : book.workData.description[0]?.value}</p>
        ...
        <p></p>
          {book.physical_format && (
            <p className={styles.label}>Format physique : {book.physical_format}</p>
        )}
      </div>
    </div>
    </>
